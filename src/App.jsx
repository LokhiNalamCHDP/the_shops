...
function AppContent() {
  ...
  const movieSlides = useMemo(
   ...
   () => films.map(film => ({
      title: film.Title,
      image: film.FilmPosterUrl || film.FilmPosterThumbnailUrl,
  ...
  useEffect(() => {
    ...
    const fetchVeeziData = async () => {
      ...
      try {
        ...
        // Handle both single film object and array of films

        const filmsArray = Array.isArray(filmsData) ? filmsData : [filmsData]

        filmsArray.forEach(film => {

          console.log('FilmPosterUrl for', film.Title + ':', film.FilmPosterUrl)

        })

        // Store films in state
                ...
                <div className="relative mt-10" onMouseEnter={() => setIsMoviePaused(true)} onMouseLeave={() => setIsMoviePaused(false)}>
                  ...
                  >
                    ...
                    {movieSlides.map((slide, i) => {
                      ...
                      const translateX = Math.max(-150, Math.min(150, rel * 180))
